      ...
      <section className="hero-bg pt-36 pb-20 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <p className="text-white/60 text-lg max-w-2xl mx-auto">{cp.hero.subtitle}</p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-5 gap-12">
                ...
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        <label className="block text-xs font-semibold text-charcoal-600 dark:text-white/60 mb-1.5">{cp.form.name} *</label>
                        <input required name="name" value={form.name} onChange={handleChange} className={inputClass} placeholder={isRTL ? 'أحمد الشمري' : 'Ahmed Al-Shamri'} />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-charcoal-600 dark:text-white/60 mb-1.5">{cp.form.company} *</label>
                        <input required name="company" value={form.company} onChange={handleChange} className={inputClass} placeholder={isRTL ? 'اسم الشركة أو الجهة' : 'Company Name'} />
                      </div>
                    </div>

                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        <label className="block text-xs font-semibold text-charcoal-600 dark:text-white/60 mb-1.5">{cp.form.email} *</label>
                        <input required type="email" name="email" value={form.email} onChange={handleChange} className={inputClass} placeholder="info@company.com" dir="ltr" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-charcoal-600 dark:text-white/60 mb-1.5">{cp.form.phone}</label>
                        <input type="tel" name="phone" value={form.phone} onChange={handleChange} className={inputClass} placeholder="+966 5X XXX XXXX" dir="ltr" />
                      </div>
                    </div>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-charcoal-600 dark:text-white/60 mb-1.5">{cp.form.message}</label>
                      <textarea
                        name="message"
                        value={form.message}
                        ...
                        placeholder={isRTL ? 'اكتب تفاصيل إضافية عن احتياجاتك...' : 'Tell us more about your needs...'}
                      />
                    </div>
            ...
            >
              ...
              <div className="bg-teal-950 rounded-3xl p-7 border border-gold-500/15">
                <h2 className="text-white font-bold text-lg mb-6 gold-line-before">{cp.info.title}</h2>
                <div className="space-y-5">
                  {infoItems.map(({ Icon, label, dir }) => (
                    <div key={label} className="flex items-start gap-3">
                      ...
                      <span className="text-white/60 text-sm leading-relaxed" dir={dir}>{label}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white dark:bg-charcoal-800 rounded-3xl overflow-hidden border border-gray-100 dark:border-white/8 shadow-card h-64 relative">
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-teal-100 to-teal-200 dark:from-teal-900/50 dark:to-teal-800/50 flex items-center justify-center">
                  <div className="text-center">
                    <MapPin size={32} className="text-teal-600 dark:text-teal-400 mx-auto mb-2" />
